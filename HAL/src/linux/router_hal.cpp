...
bool inited = false;// 并没有被init
int debugEnabled = 0;// 不可以debug
in_addr_t interface_addrs[N_IFACE_ON_BOARD] = {0};
macaddr_t interface_mac[N_IFACE_ON_BOARD] = {0};

pcap_t *pcap_in_handles[N_IFACE_ON_BOARD];// handle中的pcap
pcap_t *pcap_out_handles[N_IFACE_ON_BOARD];

std::map<std::pair<in_addr_t, int>, macaddr_t> arp_table;// arp_桌
std::map<std::pair<in_addr_t, int>, uint64_t> arp_timer;//arp_时间存放

extern "C" {
int HAL_Init(int debug, in_addr_t if_addrs[N_IFACE_ON_BOARD]) {// HAL_Init
  if (inited) {//如果init了，就跳到0
    return 0;
  }
  debugEnabled = debug;//
...
int HAL_ArpGetMacAddress(int if_index, in_addr_t ip, macaddr_t o_mac) {
  ...
  if ((ip & 0xe0) == 0xe0) {
    ...
    return 0;// 如果是广播的话,那么就把multicasting的
  }
